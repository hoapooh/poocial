	...
	useEffect(() => {
		const fetchNotifications = async () => {
			try {
				...
				// NOTE: Set the notifications state with the data
				setNotifications(data)

				// NOTE: Look for notifications that are unread
				const unreadIds = data.filter((n) => !n.read).map((n) => n.id)

				// NOTE: If there are unread notifications, mark them as read
				// NOTE: Here the date of notifications are already saved to the notifications state so that only if reload, the unread will be marked as read
